-- Fixed Supabase Auth Integration Schema
...
-- Drop existing policies first to avoid recursion issues
...
-- Drop existing tables (CASCADE will automatically drop remaining policies, triggers, and indexes)
...
-- Row Level Security (RLS) policies - Fixed to prevent infinite recursion
ALTER TABLE users ENABLE ROW LEVEL SECURITY;
ALTER TABLE categories ENABLE ROW LEVEL SECURITY;
ALTER TABLE expenses ENABLE ROW LEVEL SECURITY;
ALTER TABLE login_activities ENABLE ROW LEVEL SECURITY;

-- Fixed RLS policies using security definer functions to avoid recursion
...
-- Expenses policies using the helper function
...
COMMENT ON FUNCTION user_role(UUID) IS 'Helper function to get user role without recursion';
