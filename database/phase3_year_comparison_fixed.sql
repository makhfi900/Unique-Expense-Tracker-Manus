-- Phase 3: Year-over-Year Comparison Enhancement (FIXED VERSION)
...
BEGIN
    ...
    SELECT 
        COALESCE(SUM(CASE WHEN year = p_base_year THEN total_amount END), 0),
        COALESCE(SUM(CASE WHEN year = p_compare_year THEN total_amount END), 0)
    INTO base_total, compare_total
    FROM mv_yearly_monthly_breakdown
    WHERE year IN (p_base_year, p_compare_year)
